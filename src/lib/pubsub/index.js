...
const { track } = require('../track')(process.env.GCLOUD_PROJECT, process.env.GCLOUD_DATASTORE_NAMESPACE)
 ...
 */
async function emit(userId, eventName, payload = null, asynchronous = true, timer = 0) {
 ...
 * @param {*} payload 
 */
const executeSequentially = (arrPromises, payload, timer = 1000, results = [], indc = 0) => {
  return new Promise(async resolve => {
    if (arrPromises[indc]) {
      ...
      if (arrPromises.length <= indc) {
        ...
        setTimeout(() => {
          resolve(executeSequentially(arrPromises, payload, timer, results, indc + 1))
        }, timer)
      }
    } else {
      resolve(results)
...
/**
 * Emit an event with Cloud Task.
 * @param {*} userId 
 * @param {*} eventName 
 * @param {*} payload 
 * @param {*} executeAt 
 */
function emitAsync(userId, eventName, payload, executeAt = undefined) {
  const newPayload = { payload, eventName, userId }

  console.log("emitAsync", eventName, "schedule at?", executeAt)
  if (process.env.NODE_ENV != "test") console.log('Async event fired:', eventName)
  let res

  if (process.env.NODE_ENV != "test") {
    res = createTask("/cloud-tasks", newPayload, executeAt)
  }
